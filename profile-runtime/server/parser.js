        ...
        case 14: { // Mob-type commands
            ...
            if (subType === 1) { // tppos
                ...
                for (const id in ENTITIES.MOBS) {
                    ...
                    if (!m || m.type !== mobType) continue;
                    cmdRun.tppos(2, m.id, x, y);
                }
                ...
                for (const id in ENTITIES.MOBS) {
                    const m = ENTITIES.MOBS[id];
                    if (!m || m.type !== mobType) continue;
                    cmdRun.tpent(2, m.id, targetType, targetId);
                }
                ...
                for (const id in ENTITIES.MOBS) {
                    const m = ENTITIES.MOBS[id];
                    if (!m || m.type !== mobType) continue;
                    cmdRun.setattr(2, m.id, attrIdx, value);
                }
                ...
                for (const id in ENTITIES.MOBS) {
                    const m = ENTITIES.MOBS[id];
                    if (!m || m.type !== mobType) continue;
                    cmdRun.damage(2, m.id, damage, isPercentage);
                }
            }
            break;
        }
