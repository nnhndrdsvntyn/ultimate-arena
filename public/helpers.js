...
export function encodeUsername(usernameStr, authToken = '', world = 'tutorial') {
    writer.reset();
    writer.writeU8(1);
    writer.writeStr(usernameStr);
    writer.writeStr(String(authToken || ''));
    writer.writeStr(String(world || 'tutorial'));
...
export function sendCoinSnapshotRequestPacket() {
    if (!ws || ws.readyState !== ws.OPEN) return false;
    writer.reset();
    writer.writeU8(40);
    ws.send(writer.getBuffer());
...
export function sendTpDimCommand(entityType, startId, endId, dimensionTarget) {
    uiState.dimensionTransitionUntil = performance.now() + 1500;
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(26); // tpdim type
    writer.writeU8(entityType);
    writer.writeU16(startId);
    writer.writeU16(endId);
    writer.writeStr(dimensionTarget);
    ws.send(writer.getBuffer());
}
...
export function sendGrantAccountAdminCommand(username) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(27); // persistent account admin command type
    writer.writeStr(String(username || '').trim());
    ws.send(writer.getBuffer());
}
...
export function sendYetiAbilityCommand(abilityNumber) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(25); // yeti ability command type
    writer.writeU8(Math.max(1, Math.min(4, Math.round(Number(abilityNumber) || 1))));
    ws.send(writer.getBuffer());
}

export function sendDuneBehemothAbilityCommand(abilityNumber) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(28); // dune behemoth ability command type
    writer.writeU8(Math.max(1, Math.min(4, Math.round(Number(abilityNumber) || 1))));
    ws.send(writer.getBuffer());
}

export function sendInfernoBeastAbilityCommand(abilityNumber) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(29); // inferno beast ability command type
    writer.writeU8(Math.max(1, Math.min(4, Math.round(Number(abilityNumber) || 1))));
    ws.send(writer.getBuffer());
}

export function sendAdminCreativeItemCommand(itemType, amount = 1, slot = 255, drop = false) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(21); // creative item command type
    writer.writeU8(Math.max(0, Math.min(255, Math.round(itemType))));
    writer.writeU16(Math.max(1, Math.min(65535, Math.round(amount))));
    writer.writeU8(Math.max(0, Math.min(255, Math.round(slot))));
    writer.writeU8(drop ? 1 : 0);
    ws.send(writer.getBuffer());
}

export function sendGiveItemCommand(entityType, startId, endId, itemType, amount = 1) {
    writer.reset();
    writer.writeU8(8); // Command packet
    writer.writeU8(24); // give item command type
    writer.writeU8(entityType);
    writer.writeU16(startId);
    writer.writeU16(endId);
    writer.writeU8(Math.max(0, Math.min(255, Math.round(itemType))));
...
export function sendUseItemPacket() {
    writer.reset();
    writer.writeU8(29); // use/consume item packet
    ws.send(writer.getBuffer());
}
...
export function sendUiPanelVisibilityPacket(panelId, isOpen) {
    if (!ws || ws.readyState !== ws.OPEN) return;
    writer.reset();
    writer.writeU8(33); // Type 33: UI panel visibility
    writer.writeU8(Math.max(0, Math.min(255, Math.floor(panelId || 0))));
    writer.writeU8(isOpen ? 1 : 0);
    ws.send(writer.getBuffer());
}
