...
async function loadAssets() {
    ...
    if (IS_FIRST_TUTORIAL_RUN) {
        ...
        for (const chestId of (dataMap.CHEST_IDS || [])) {
            if (dataMap.OBJECTS[chestId]) tutorialObjects[chestId] = dataMap.OBJECTS[chestId];
        }
...
function worldToScreenPos(worldX, worldY) {
    ...
    const outW = tutorialIndicatorUi.width || window.innerWidth || LC.width;
    const outH = tutorialIndicatorUi.height || window.innerHeight || LC.height;
    const mappedX = LC.width > 0 ? (logicalScreenX / LC.width) * outW : logicalScreenX;
    const mappedY = LC.height > 0 ? (logicalScreenY / LC.height) * outH : logicalScreenY;
    return {
        screenX: mappedX,
        screenY: mappedY
    };
