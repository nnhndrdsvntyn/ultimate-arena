...
export const Vars = {
    ...
    dragSlot: -1,
    lastSelectionTime: 0,
    myStats: { dmgHit: 0, dmgThrow: 0, speed: 0, hp: 100, maxHp: 100 }
};
...
function drawLoadingScreen() {
    ...
    LC.drawRect({ pos: [x, y], size: [barWidth * loadingState.progress, barHeight], color: '#38bdf8', cornerRadius: 3 });
    ...
    LC.drawText({ text: `${Math.floor(loadingState.progress * 100)}%`, pos: [LC.width / 2, y + 25], font: '600 12px Inter, sans-serif', color: 'rgba(56, 189, 248, 0.5)', textAlign: 'center' });
...
function drawInfoBox(lp) {
    ...
    LC.drawRect({ pos: [5, 5], size: [150, 110], color: 'rgba(0, 0, 0, 0.4)', cornerRadius: 5 });
    text.forEach((t, i) => LC.drawText({ text: t, pos: [15, 25 + i * 25], font: '16px Inter', color: 'white' }));
}
...
function drawMinimap() {
    const isMobileUI = isMobile || Settings.forceMobileUI;
    const size = 180, x = LC.width - size - 20, y = LC.height - size - (isMobileUI ? 180 : 40);
    ...
    if (Settings.showChestsOnMinimap) Object.values(ENTITIES.OBJECTS).filter(o => o.type <= 4).forEach(o => drawDot(o, '#b45309'));
...
function drawHotbar() {
    const hb = HOTBAR_CONFIG, totalW = (hb.slotSize * 3) + (hb.gap * 2) + (hb.padding * 2);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        LC.drawRect({ pos: [sx, sy], size: [hb.slotSize, hb.slotSize], color: selected ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.2)', cornerRadius: 8, stroke: selected ? '#38bdf8' : 'rgba(255,255,255,0.1)', strokeWidth: selected ? 2 : 1 });

        let rank = Vars.myInventory[i];
        if (rank > 0 && i !== Vars.dragSlot) {
            const isThrown = rank > 127;
            const lookupRank = isThrown ? rank & 0x7F : rank;
            const sword = dataMap.SWORDS.imgs[lookupRank] || dataMap.SWORDS.imgs[1];
...
function drawMobileButtons(lp) {
    ...
    drawBtn(ATTACK_BTN_CONFIG, 'ATTACK', true);
