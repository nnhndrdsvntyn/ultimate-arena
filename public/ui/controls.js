...
import { ws, Vars, LC, camera, isTutorialMobileActionEnabled, getHudUpgradeTypeAtClientPos, isHudUpgradeHeaderAtClientPos, tryUseHudUpgradeAtClientPos, getCreativeInventorySlotAtLogicalPos, getCreativeInventoryItemBySlot, getAdminInventoryPanelPositions } from '../client.js';
import { writer, sendPickupCommand, sendEquipAccessoryPacket, sendUseAbilityPacket, sendAdminCreativeItemCommand } from '../helpers.js';
...
function handleCreativeInventoryDrop(clientX, clientY) {
    ...
    const hotbarSlot = isClickingHotbar(clientX, clientY);
    const invSlot = isClickingInventory(clientX, clientY);
    ...
    window.addEventListener('touchstart', (e) => {
        ...
        Array.from(e.changedTouches).forEach(t => {
            ...
            if (isTouchOnUI(t.clientX, t.clientY)) {
                ...
                const creativeSlot = getCreativeSlotUnderClient(t.clientX, t.clientY);
                if (creativeSlot !== -1) {
...
function isButtonTouched(clientX, clientY, config) {
    const { x: sx, y: sy } = LC.clientToLogical(clientX, clientY);
...
export function setupDesktopControls() {
    window.addEventListener("mousemove", (e) => {
        ...
        window.addEventListener("mousedown", (e) => {
            ...
            const creativeSlot = getCreativeSlotUnderClient(e.clientX, e.clientY);
            if (creativeSlot !== -1) {
                const creativeItem = getCreativeInventoryItemBySlot(creativeSlot);
                if (creativeItem) {
                    Vars.creativeDragItemType = creativeItem.type;
