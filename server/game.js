...
export function buildInitPacket(wsId, world = 'main') {
    ...
    const players = Object.values(ENTITIES.PLAYERS).filter(player => (player.world || 'main') === world);
...
export function spawnObject(type, x, y, amount = 1, source = null, world = null) {
    ...
    if (x === undefined || y === undefined) {
        ...
        while (!validPosition) {
            ...
            const spawnZone = Object.values(ENTITIES.STRUCTURES).find(s => s.type === 1 && (s.world || 'main') === worldId);
            ...
            for (const struct of Object.values(ENTITIES.STRUCTURES)) {
                if ((struct.world || 'main') !== worldId) continue;
