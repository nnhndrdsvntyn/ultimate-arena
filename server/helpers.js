...
import {
    ...
    markRootWalkerDimensionEntry,
    markYetiDimensionEntry,
    markDuneDimensionEntry,
    markInfernoDimensionEntry,
    ...
    getDuneBoss,
    getInfernoBoss,
    hasRemainingRootWalkerLoot,
    spawnRootWalkerBoss,
    spawnYetiBoss,
    spawnDuneBoss,
    spawnInfernoBoss,
    BOSS_DIMENSION_SPAWN,
    BOSS_INTRO_COUNTDOWN_MS,
    BOSS_INTRO_ROV,
    BOSS_FIGHT_ROV,
    beginBossIntroForWorld,
    getBossIntroUntilForWorld
} from './game.js';
import fs from 'fs';
const PACKET_STRUCTURE_ADD = 28;
const PACKET_STRUCTURE_REMOVE = 29;
const PACKET_BOSS_INTRO_COUNTDOWN = 41;
const PACKET_GROUND_LOOT_CLEAR = 43;
const PACKET_INFERNO_BEAM_FX = 44;
import {
    wss
} from '../server.js';
import {
...
import { getCenterDiagonalBridgeSegments } from '../public/shared/river.js';
import {
    getWorldCenter,
    getWorldMapSize,
    normalizeDimensionTarget,
    worldHasRivers
...
function isBossDimension(worldId) {
    return worldId === 'root_dimension' || worldId === 'yeti_dimension' || worldId === 'dune_dimension' || worldId === 'inferno_dimension';
}

function applyPlayerViewRangeOverride(player, rangeMult) {
    if (!player || !Number.isFinite(rangeMult) || rangeMult <= 0) return;
    const clamped = Math.max(0.1, Math.min(rangeMult, 10));
    player.viewRangeOverride = clamped;
    player.viewRangeMult = clamped;
    const client = [...wss.clients].find(c => c.id === player.id);
    if (client) client.viewRangeMult = clamped;
}

function sendBossIntroCountdown(player, remainingMs) {
    const client = [...wss.clients].find(c => c.id === player?.id);
    if (!client || client.readyState !== 1) return;
    const pw = new PacketWriter(8);
    pw.writeU8(PACKET_BOSS_INTRO_COUNTDOWN);
    pw.writeU16(Math.max(0, Math.min(65535, Math.ceil(remainingMs))));
    pw.writeU8(Math.max(1, Math.min(10, Math.round(BOSS_INTRO_ROV))));
    pw.writeU8(Math.max(1, Math.min(10, Math.round(BOSS_FIGHT_ROV))));
    client.send(pw.getBuffer());
}

function broadcastGroundLootClear() {
    const pw = new PacketWriter(8);
    pw.writeU8(PACKET_GROUND_LOOT_CLEAR);
    const packet = pw.getBuffer();
...
function applyBossIntroToPlayer(player, worldId, now = performance.now()) {
    if (!player || !isBossDimension(worldId)) return;
    const startedUntil = beginBossIntroForWorld(worldId, now);
    const introUntil = startedUntil || getBossIntroUntilForWorld(worldId);
    if (introUntil <= now) return;

    player.frozenUntil = Math.max(player.frozenUntil || 0, introUntil);
    player.swingState = 0;
    player.attacking = false;
    player._forceFullSync = true;
    applyPlayerViewRangeOverride(player, BOSS_INTRO_ROV);
    sendBossIntroCountdown(player, introUntil - now);

    setTimeout(() => {
        if (!player || (player.world || 'main') !== worldId) return;
        applyPlayerViewRangeOverride(player, BOSS_FIGHT_ROV);
        player.sendStatsUpdate?.();
    }, Math.max(0, Math.ceil(introUntil - now)) + 25);
}
...
export function getWorldEnvironmentData(world = 'main') {
    ...
    if (cached) {
        ...
        if (valid) {
            for (let i = 0; i < cached.treeIds.length; i++) {
                if (!ENTITIES.STRUCTURES[cached.treeIds[i]]) {
                    valid = false;
                    break;
                }
            }
    ...
    if (!worldHasRivers(worldId)) {
        const entry = {
            ...
            bridgeBandsX: [],
            bridgeBands: [],
            diagonalBridgeHalfWidth: 0,
            diagonalBridgeSegments: []
        };
    ...
    const segmentHeight = mapSize[1] / (bridgeCount + 1);
    const segmentWidth = mapSize[0] / (bridgeCount + 1);
    const bridgeBandsY = [];
    const bridgeBandsX = [];
    for (let i = 1; i <= bridgeCount; i++) {
        ...
        bridgeBandsX.push({
            minX: bridgeCenterX - bridgeHalfHeight,
            maxX: bridgeCenterX + bridgeHalfHeight
        });
    }
    const safeZoneRadius = Math.max(1, Math.floor(safeZone?.radius || safeZoneCfg.radius || safeZoneCfg.safeZoneHalfSize || 500));
    const diagonalBridgeHalfWidth = Math.max(8, Math.floor(safeZoneCfg.diagonalBridgeHalfWidth || bridgeHalfHeight));
    const diagonalBridgeSegments = getCenterDiagonalBridgeSegments(mapSize, safeZoneRadius, { landPad: 24, step: 8 });
    ...
    for (const id in ENTITIES.STRUCTURES) {
        ...
        if (!structure) continue;
        if ((structure.world || 'main') !== worldId) continue;
        if (structure.type === 3) treeIds.push(structure.id);
    }
    ...
    const entry = {
        ...
        bridgeBandsY,
        bridgeBandsX,
        bridgeBands: bridgeBandsY,
        diagonalBridgeHalfWidth,
        diagonalBridgeSegments
    };
    worldEnvironmentCache.set(worldId, entry);
    return {
...
export function clearWorldCaches(world) {
    if (!world) return;
    safeZoneStructureCache.delete(world);
    worldEnvironmentCache.delete(world);
    if (queuedCoinPickupFx.length) {
        for (let i = queuedCoinPickupFx.length - 1; i >= 0; i--) {
            if (queuedCoinPickupFx[i].world === world) queuedCoinPickupFx.splice(i, 1);
        }
    }
    if (queuedDamageIndicatorFx.length) {
        for (let i = queuedDamageIndicatorFx.length - 1; i >= 0; i--) {
            if (queuedDamageIndicatorFx[i].world === world) queuedDamageIndicatorFx.splice(i, 1);
        }
    }
    if (queuedChestCoinSeeds.length) {
        for (let i = queuedChestCoinSeeds.length - 1; i >= 0; i--) {
            if (queuedChestCoinSeeds[i].world === world) queuedChestCoinSeeds.splice(i, 1);
        }
    }
}
...
export function playSfx(xorigin, yorigin, type, range, world = 'main') {
    const rangeSqrd = range * range;
    const sourceWorld = world || 'main';

    wss.clients.forEach(client => {
        if (client.readyState !== 1) return;
        const player = ENTITIES.PLAYERS[client.id];
        if (!player) return;
        if ((player.world || 'main') !== sourceWorld) return;
...
export function emitPoisonAoeFx(x, y, radius = 300, durationMs = 700, waves = 2, world = 'main', colorCode = 0) {
    const fxWriter = new PacketWriter(16);
    fxWriter.reset();
    ...
    fxWriter.writeU16(Math.max(1, Math.min(10000, Math.round(durationMs))));
    fxWriter.writeU8(Math.max(1, Math.min(8, Math.round(waves))));
    fxWriter.writeU8(Math.max(0, Math.min(255, Math.round(colorCode))));
    const packet = fxWriter.getBuffer();
    forEachOpenClientInWorld(world, client => {
        client.send(packet);
    });
...
export function emitInfernoBeamFx(x, y, angle, length = 1100, width = 220, chargeMs = 3000, collapseMs = 100, beamMs = 420, world = 'main', ownerId = null, targetPlayerId = null) {
    const fxWriter = new PacketWriter(24);
    fxWriter.reset();
    fxWriter.writeU8(PACKET_INFERNO_BEAM_FX);
    fxWriter.writeU16(Math.max(0, Math.min(65535, Math.round(x))));
    fxWriter.writeU16(Math.max(0, Math.min(65535, Math.round(y))));
    fxWriter.writeF32(Number.isFinite(angle) ? angle : 0);
    fxWriter.writeU16(Math.max(1, Math.min(65535, Math.round(length))));
    fxWriter.writeU16(Math.max(1, Math.min(65535, Math.round(width))));
    fxWriter.writeU16(Math.max(1, Math.min(10000, Math.round(chargeMs))));
    fxWriter.writeU16(Math.max(1, Math.min(1000, Math.round(collapseMs))));
    fxWriter.writeU16(Math.max(1, Math.min(5000, Math.round(beamMs))));
    fxWriter.writeU16(Number.isFinite(ownerId) ? Math.max(0, Math.min(65535, Math.round(ownerId))) : 65535);
    fxWriter.writeU8(Number.isFinite(targetPlayerId) ? Math.max(0, Math.min(255, Math.round(targetPlayerId))) : 255);
    const packet = fxWriter.getBuffer();
    forEachOpenClientInWorld(world, client => {
        client.send(packet);
    });
...
export function emitHeartMistFx(playerId, durationMs = 3000, radius = 70, world = 'main') {
    const fxWriter = new PacketWriter(10);
    fxWriter.reset();
    fxWriter.writeU8(34); // Heart mist effect
    fxWriter.writeU8(Math.max(0, Math.min(255, Math.floor(playerId))));
    fxWriter.writeU16(Math.max(1, Math.min(65535, Math.round(durationMs))));
    fxWriter.writeU16(Math.max(1, Math.min(65535, Math.round(radius))));
    const packet = fxWriter.getBuffer();
    forEachOpenClientInWorld(world, client => {
...
export function spawnEnergyBurstProjectiles(source, options = {}) {
    ...
    const projectileRange = dataMap.PROJECTILES[13]?.maxDistance || 500;
...
export function poison(entity, dmgPerRate, rate, duration, attacker = null, noKillCredit = true) {
    ...
    const getEffectivePoisonTickRate = () => {
        // Poison ticks twice as fast while hidden in a tree.
        if (entity?.isHidden) return Math.max(1, Math.floor(rate / 2));
        return rate;
    };
    ...
    const tick = () => {
        ...
        entity._poison.timer = setTimeout(tick, getEffectivePoisonTickRate());
    };
...
class CommandMap {
    ...
    markTeleported(entityType, entity) {
        ...
        const seenKeyBase = entityType === 1 ? ENTITY_KEY_PLAYER : (entityType === 2 ? ENTITY_KEY_MOB : 0);
        if (!seenKeyBase) return;
        const seenKey = seenKeyBase + entity.id;
        for (const client of wss.clients) {
    ...
    spawn(entityKey, x, y, world = 'main') {
        ...
        const clampedX = Math.max(0, Math.min(mapSize[0], Math.round(Number.isFinite(x) ? x : 0)));
        const clampedY = Math.max(0, Math.min(mapSize[1], Math.round(Number.isFinite(y) ? y : 0)));
        const worldId = world || 'main';
        ...
        if (SPAWNABLE_OBJECT_MAP[key]) {
            spawnObject(SPAWNABLE_OBJECT_MAP[key], clampedX, clampedY, 1, null, worldId);
            return true;
        }
    ...
    tpdim(entityType, entityId, dimensionTarget) {
        if (entityType !== 1) return;
        const worldId = normalizeDimensionTarget(dimensionTarget);
        if (!worldId) return;
        const movePlayer = (player) => {
            if (!player) return;
            const previousWorld = player.world || 'main';
            if (previousWorld === worldId) return;
            const center = isBossDimension(worldId) ? BOSS_DIMENSION_SPAWN : getWorldCenter(worldId);
            if (worldId === 'root_dimension') {
                ensureRootDimensionEdgeTrees();
                if (isRootWalkerEncounterOpen() && !getRootWalkerBoss() && !hasRemainingRootWalkerLoot()) {
                    spawnRootWalkerBoss();
                }
                markRootWalkerDimensionEntry();
            } else if (worldId === 'yeti_dimension') {
                ensureYetiDimensionSnowStructures();
                if (isYetiEncounterOpen() && !getYetiBoss()) {
                    spawnYetiBoss();
                }
                markYetiDimensionEntry();
            } else if (worldId === 'dune_dimension') {
                ensureDuneDimensionDesertStructures();
                if (isDuneEncounterOpen() && !getDuneBoss()) {
                    spawnDuneBoss();
                }
                markDuneDimensionEntry();
            } else if (worldId === 'inferno_dimension') {
                ensureInfernoDimensionMagmaStructures();
                if (isInfernoEncounterOpen() && !getInfernoBoss()) {
                    spawnInfernoBoss();
                }
                markInfernoDimensionEntry();
            }
            ...
            player.x = center.x;
            player.y = center.y;
            player.lastX = center.x;
            player.lastY = center.y;
            player._forceFullSync = true;
            const client = [...wss.clients].find(c => c.id === player.id);
            if (client) {
                client.world = worldId;
                client.seenEntities = new Set();
                client._pendingSeenEntities = new Set();
                client._lastOptionalSyncWorld = null;
                client._lastTopLeaderSignature = null;
                client._lastMinimapSignature = null;
                client._minimapTickCounter = 0;
                client.send(buildInitPacket(client.id, worldId));
            }
            applyBossIntroToPlayer(player, worldId);
            const seenKey = ENTITY_KEY_PLAYER + player.id;
            for (const otherClient of wss.clients) {
                if (otherClient?.seenEntities) otherClient.seenEntities.delete(seenKey);
                if (otherClient?._pendingSeenEntities instanceof Set) otherClient._pendingSeenEntities.delete(seenKey);
            }
        };

        if (entityId === 0) {
            for (const id in ENTITIES.PLAYERS) {
                movePlayer(ENTITIES.PLAYERS[id]);
            }
            return;
        }

        movePlayer(ENTITIES.PLAYERS[entityId]);
    }
    ...
    setattr(entityType, entityId, attrIdx, value) {
        ...
        if (entity && entityType === 1) { // Player
            ...
            if (attrIdx === 1) { // speed
                if (isDefault) {
                    mob.speedOverride = null;
                } else {
                    mob.speedOverride = Math.max(0, value);
                }
                const baseSpeed = Number.isFinite(mob.speedOverride) ? mob.speedOverride : (dataMap.MOBS[mob.type]?.speed || mob.speed || 0);
                const speedMult = mob.isAlarmed && typeof mob.getAlarmSpeedMultiplier === 'function' ? mob.getAlarmSpeedMultiplier() : 1;
                mob.speed = baseSpeed * speedMult;
            } else if (attrIdx === 5) { // strength
                mob.strength = isDefault ? 0 : Math.floor(value);
            } else if (attrIdx === 7) { // invincible
                mob.invincible = isDefault ? false : !!value;
            } else if (attrIdx === 6) { // maxHealth
                const nextMax = isDefault ? (dataMap.MOBS[mob.type]?.baseHealth || mob.maxHp || 1) : Math.max(1, Math.floor(value));
                const ratio = (mob.maxHp > 0) ? (mob.hp / mob.maxHp) : 1;
                mob.maxHp = nextMax;
                mob.hp = Math.max(0, Math.min(mob.maxHp, Math.round(mob.maxHp * ratio)));
            } else if (attrIdx === 9) { // radius
    ...
    moveInventoryItemToFreeSlotOrDrop(player, itemType, count, excludedSlot = -1) {
        ...
        if (objectCfg?.stackable) {
            ...
            for (let i = 0; i < player.inventory.length; i++) {
                ...
                if (player.inventory[i] !== itemType) continue;
                if (player.inventoryCounts[i] >= stackLimit) continue;
                const space = Math.max(0, stackLimit - (player.inventoryCounts[i] || 0));
                if (space <= 0) continue;
            ...
            while (remaining > 0) {
                const toDrop = Math.min(stackLimit, remaining);
                const dropObj = spawnObject(itemType, player.x, player.y, toDrop, 'player', player.world || 'main');
    ...
    creativeItem(entityId, itemType, amount = 1, targetSlot = 255, drop = false) {
        ...
        const objectCfg = dataMap.OBJECTS?.[normalizedType];
        const stackLimit = Math.max(1, Math.floor(objectCfg?.stackLimit || 256));
        const normalizedAmount = isCoinObjectType(normalizedType)
            ? Math.max(1, Math.min(256, Math.floor(amount || 1)))
            : (objectCfg?.stackable ? Math.max(1, Math.min(stackLimit, Math.floor(amount || 1))) : 1);
        const isValidType = isWeaponRank(normalizedType)
            || isAccessoryItemType(normalizedType)
            || isCoinObjectType(normalizedType)
            || !!objectCfg?.stackable
            || !!objectCfg?.isEphemeral;
        if (!isValidType) return;

        if (drop) {
            if (typeof player.canDropItems === 'function' && !player.canDropItems()) return;
    ...
    giveItem(entityId, itemType, amount = 1) {
        const player = ENTITIES.PLAYERS[entityId];
        if (!player || !player.isAlive) return;
        ...
        const objectCfg = dataMap.OBJECTS?.[normalizedType];
        const stackLimit = Math.max(1, Math.floor(objectCfg?.stackLimit || 256));
        const normalizedAmount = (isCoinObjectType(normalizedType) || objectCfg?.stackable)
            ? Math.max(1, Math.floor(amount || 1))
            : 1;
        const isValidType = isWeaponRank(normalizedType)
            || isAccessoryItemType(normalizedType)
            || isCoinObjectType(normalizedType)
        ...
        if (objectCfg?.stackable && normalizedAmount > stackLimit) {
            ...
            while (remaining > 0) {
                const toAdd = Math.min(stackLimit, remaining);
                this.moveInventoryItemToFreeSlotOrDrop(player, normalizedType, toAdd);
                remaining -= toAdd;
            }
        } else {
            this.moveInventoryItemToFreeSlotOrDrop(player, normalizedType, normalizedAmount);
    ...
    resetServer(seed = null) {
        if (Number.isFinite(seed) && seed >= 0 && seed <= 0xFFFFFFFF) {
            try {
                ...
                fs.writeFileSync('./profile_runtime/last_seed.txt', String(seed >>> 0), 'utf8');
            } catch (e) {
    ...
    activateAbility(playerId, abilityName, targetX = null, targetY = null, durationSeconds = null) {
        ...
        if (ability !== 'energy_burst'
            && ability !== 'lightning_shot'
            && ability !== 'stamina_boost'
            && ability !== 'speed_boost'
            && ability !== 'poison_blast'
            && ability !== 'smoke_blast'
            && ability !== 'growth_spurt'
            && ability !== 'invisibility'
            && ability !== 'burst_heal') return;
        ...
        if (ability === 'burst_heal') {
            player.activateHeartShadesBurst(3000);
            emitHeartMistFx(player.id, 3000, 70, player.world || 'main');
            return;
        }
        ...
        if (ability !== 'poison_blast' && ability !== 'smoke_blast') {
            ...
            if (electricSfx >= 0) {
                playSfx(player.x, player.y, electricSfx, 1200, player.world || 'main');
            }
        }
    ...
    y(abilityNumber) {
        ...
        for (const id in ENTITIES.MOBS) {
            ...
            if (typeof mob.activateAbility === 'function') {
                mob.activateAbility(ability, performance.now(), { respectCooldowns: false });
            }
        }
    }
