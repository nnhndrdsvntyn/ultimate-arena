...
export function sendUpdates(wss, lbWriter) {
    wss.clients.forEach(ws => {
        ...
        writePlayers(pw, ws, lpX, lpY, renderDistanceSq, isFull, entitiesInUpdate, world);
        ...
        if (shouldSendLeaderboard(ws)) {
            const leaderboard = getLeaderboard(world);
...
function getLeaderboard(world) {
    const lb = Object.values(ENTITIES.PLAYERS)
        .filter(p => p.isAlive && (p.world || 'main') === world)
...
function writePlayers(pw, ws, lpX, lpY, rangeSq, isFullFn, entities, world) {
...
function writeMobs(pw, lpX, lpY, rangeSq, isFullFn, entities, world, forceWorldSync = false) {
...
function writeProjectiles(pw, lpX, lpY, rangeSq, isFullFn, entities, world, forceWorldSync = false) {
    const countPos = pw.reserveU16();
    let count = 0;
...
function writeObjects(pw, lpX, lpY, rangeSq, isFullFn, entities, world, forceWorldSync = false) {
    const countPos = pw.reserveU16();
    let count = 0;
